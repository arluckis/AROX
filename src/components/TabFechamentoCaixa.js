...
export default function TabFechamentoCaixa({ temaNoturno, sessao, caixaAtual, comandas, fetchData, mostrarAlerta, mostrarConfirmacao }) {
  ...
  // Se o usuário entrar na aba e o caixa já estiver fechado, pede a senha na mesma hora
  useEffect(() => {
  ...
  const excluirCaixaConfirmado = async () => {
    ...
    if (!error) { 
      mostrarAlerta("Sucesso", "Fechamento de caixa excluído permanentemente."); 
      carregarHistorico(); 
    } else { 
      mostrarAlerta("Erro", "Erro ao excluir caixa: " + error.message); 
    }
  };
  ...
  const handleVerificarSenha = async () => {
    ...
    if (data && data.length > 0) {
      ...
      if (acaoPendente === 'excluir_fechamento') {
        mostrarConfirmacao(
          'Atenção: Excluir Fechamento', 
          'Esta ação apagará este registro de fechamento do banco de dados e não pode ser desfeita. Deseja continuar?', 
          excluirCaixaConfirmado
        );
      }
    } else { 
      mostrarAlerta("Acesso Negado", "Senha incorreta ou você não possui permissão de dono."); 
    }
  };
  ...
  const pagarMotoboysConfirmado = async () => {
    ...
    else { carregarDadosCaixaAtual(); }
  };

  const abrirConfirmacaoMotoboy = () => {
    mostrarConfirmacao(
      'Confirmar Pagamento', 
      `Deseja registrar o repasse de R$ ${pendenteMotoboy.toFixed(2)} para os motoboys?`, 
      pagarMotoboysConfirmado
    );
  };
  ...
  const encerrarCaixaConfirmado = async () => {
    ...
    if (error) return mostrarAlerta("Erro Crítico", "ERRO NO SUPABASE:\n\n" + error.message);
    
    mostrarAlerta("Sucesso", "Turno encerrado com sucesso!");
    setValorInformadoDinheiro(''); setValorInformadoCartao(''); setValorInformadoPix('');
    setMostrarEsperado(false); 
    
    // SOLUÇÃO DEFINITIVA DO SEU PEDIDO:
    // Trava o histórico e joga a tela de SENHA antes de acessar!
  ...
  const abrirConfirmacaoFechamento = () => {
    mostrarConfirmacao(
      'Encerrar Turno', 
      'Tem certeza que deseja encerrar o turno de caixa atual?', 
      encerrarCaixaConfirmado
    );
  };
  ...
  const salvarEdicaoFechamento = async () => {
    ...
    if (!error) { 
      mostrarAlerta("Sucesso", "Apuração atualizada com sucesso!"); 
      setModalEdicao({ visivel: false, dinheiro: '', cartao: '', pix: '' }); 
      carregarHistorico(); 
    } else { 
      mostrarAlerta("Erro", "Erro ao atualizar: " + error.message); 
    }
  };
    ...
    <div className={`max-w-4xl mx-auto w-full animate-in slide-in-from-bottom-4 duration-500 p-4 rounded-3xl shadow-sm border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
      ...
      {abaInterna === 'atual' ? (
        caixaAtual?.status === 'aberto' ? (
          ...
          <div className={`p-10 text-center rounded-3xl border border-dashed flex flex-col items-center justify-center ${temaNoturno ? 'border-gray-700 text-gray-500 bg-gray-800/50' : 'border-gray-300 text-gray-400 bg-gray-50'}`}>
            ...
            <p className="font-bold text-lg text-gray-600 dark:text-gray-300">O Caixa Atual está Fechado</p>
            <p className="text-sm mt-1 max-w-sm">Navegue até a aba "Comandas" para preencher as informações e abrir um novo turno.</p>
          </div>
        )
      ) : (
        <div>
                ...
                return (
                  <div key={caixa.id} className={`relative p-5 rounded-2xl border ${temaNoturno ? 'bg-gray-900 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="absolute top-4 right-4 flex gap-3 z-10">
                      <button onClick={() => { setCaixaEditando(caixa); setAcaoPendente('editar_fechamento'); setSenhaModal({ visivel: true, senha: '' }); }} className={`text-[10px] font-bold flex items-center gap-1 uppercase tracking-widest transition hover:underline ${temaNoturno ? 'text-blue-400' : 'text-blue-600'}`}>
                      ...
                      <button onClick={() => { setCaixaEditando(caixa); setAcaoPendente('excluir_fechamento'); setSenhaModal({ visivel: true, senha: '' }); }} className={`text-[10px] font-bold flex items-center gap-1 uppercase tracking-widest transition hover:underline ${temaNoturno ? 'text-red-400' : 'text-red-600'}`}>
