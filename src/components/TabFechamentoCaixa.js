...
export default function TabFechamentoCaixa({ temaNoturno, sessao, caixaAtual, comandas, fetchData }) {
  ...
  // Novos Estados de Segurança e Correção
  const [mostrarEsperado, setMostrarEsperado] = useState(false);
  const [historicoLiberado, setHistoricoLiberado] = useState(false);
  const [acaoPendente, setAcaoPendente] = useState(null); // 'revelar', 'historico', 'editar_fechamento'
  ...
  // Formatação Blindada (Impede o Mac de subtrair 3h e cair no dia anterior)
  ...
  const carregarDadosCaixa = async () => {
    ...
    // CORREÇÃO DO MOTOBOY: Busca tudo o que foi pago a motoboys HOJE, em qualquer turno
  ...
  const handleVerificarSenha = async () => {
    ...
    if (data && data.length > 0) {
      ...
      if (acaoPendente === 'editar_fechamento') {
        ...
        setModalEdicao({
          visivel: true,
          dinheiro: rel.informadoDinheiro || '',
          cartao: rel.informadoCartao || '',
          pix: rel.informadoPix || ''
        });
      }
    } else {
      alert("Senha incorreta ou você não possui permissão de dono.");
    }
  };
  ...
  const pagarMotoboysConfirmado = async () => {
    const payload = {
      ...
      empresa_id: sessao.empresa_id,
      tipo: 'sangria',
      valor: pendenteMotoboy,
      descricao: 'Pagamento Motoboys (Entregas)'
    };
    const { data, error } = await supabase.from('caixa_movimentacoes').insert([payload]).select();
    if (data && !error) {
      setMovimentacoes(prev => [...prev, ...data]);
  ...
  const abrirConfirmacaoMotoboy = () => {
    setModalAcao({
      visivel: true,
      titulo: 'Confirmar Pagamento',
      mensagem: `Deseja registrar o repasse de R$ ${pendenteMotoboy.toFixed(2)} para os motoboys? Uma sangria automática será feita na gaveta.`,
      acao: pagarMotoboysConfirmado,
      corBotao: 'bg-blue-600 hover:bg-blue-700'
    });
  };

  const pagamentosDoTurno = comandas
    .filter(c => c.status === 'fechada')
    .flatMap(c => c.pagamentos || [])
    .filter(p => p.data?.substring(0, 10) === caixaAtual?.data_abertura?.substring(0, 10));
  ...
  const encerrarCaixaConfirmado = async () => {
    ...
    setModalAcao({ visivel: false });
    setMostrarEsperado(false); 
    fetchData(); 
  };

  const abrirConfirmacaoFechamento = () => {
    setModalAcao({
      visivel: true,
      titulo: 'Encerrar Turno',
      mensagem: 'Tem certeza que deseja encerrar o turno de caixa atual? Verifique se preencheu todos os valores de apuração corretamente.',
      acao: encerrarCaixaConfirmado,
      corBotao: 'bg-purple-600 hover:bg-purple-700'
    });
  };

  // --- FUNÇÃO PARA SALVAR A EDIÇÃO DO DONO NO HISTÓRICO ---
  const salvarEdicaoFechamento = async () => {
    ...
    const novoRelatorio = {
      ...
      informadoPix: valPix,
      diferencaDinheiro: valDinheiro - (caixaEditando.relatorio_fechamento.esperadoDinheiro || 0)
    };

    const { error } = await supabase.from('caixas')
      .update({ relatorio_fechamento: novoRelatorio })
      .eq('id', caixaEditando.id);

    if (!error) {
      alert("Apuração atualizada com sucesso!");
      setModalEdicao({ visivel: false, dinheiro: '', cartao: '', pix: '' });
      carregarDadosCaixa(); // Recarrega para mostrar os novos valores
    } else {
  ...
  return (
    <div className={`max-w-4xl mx-auto w-full animate-in slide-in-from-bottom-4 duration-500 p-4 rounded-3xl shadow-sm border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
      ...
      <div className="flex gap-4 mb-6 border-b border-gray-200/20 pb-2">
        ...
        <button onClick={() => {
            if (historicoLiberado) setAbaInterna('historico');
            else { setAcaoPendente('historico'); setSenhaModal({ visivel: true, senha: '' }); }
        }} className={`font-bold px-4 py-2 transition flex items-center gap-2 ${abaInterna === 'historico' ? (temaNoturno ? 'text-purple-400 border-b-2 border-purple-400' : 'text-purple-600 border-b-2 border-purple-600') : 'text-gray-500 hover:text-gray-400'}`}>
          Histórico de Fechamentos {!historicoLiberado && "🔒"}
        </button>
      </div>
      ...
      {abaInterna === 'atual' ? (
        <>
          ...
          <div className={`mb-6 p-5 rounded-2xl border ${temaNoturno ? 'bg-gray-900/50 border-gray-700' : 'bg-blue-50/50 border-blue-100'}`}>
            ...
            {motoboyAtivo && (
              <div className={`mt-5 p-4 rounded-xl border flex flex-col sm:flex-row justify-between items-center gap-4 ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-blue-200'}`}>
                ...
                <button onClick={abrirConfirmacaoMotoboy} disabled={pendenteMotoboy <= 0} className={`px-5 py-3 font-bold rounded-xl transition w-full sm:w-auto ${pendenteMotoboy > 0 ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md' : (temaNoturno ? 'bg-gray-700 text-gray-500' : 'bg-gray-200 text-gray-400')}`}>
                  Confirmar Pagamento
                </button>
              </div>
            )}
          ...
          <div className={`p-5 rounded-2xl border mb-6 transition-all ${temaNoturno ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200 shadow-sm'}`}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h3 className={`font-black flex items-center gap-2 ${temaNoturno ? 'text-gray-300' : 'text-gray-700'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v1a3 3 0 106 0v-1m-6 0a3 3 0 006 0M9 17h6M9 12a3 3 0 116 0 3 3 0 01-6 0z"></path></svg>
              ...
              <button onClick={() => { if(mostrarEsperado) setMostrarEsperado(false); else { setAcaoPendente('revelar'); setSenhaModal({ visivel: true, senha: '' }); } }} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs transition ${temaNoturno ? 'bg-gray-700 hover:bg-gray-600 text-gray-300' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}>
                {mostrarEsperado ? (
                  ...
                  <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg> Desbloquear</>
                )}
              </button>
            </div>
            ...
            {mostrarEsperado && (
              <div className={`mt-5 pt-5 border-t grid grid-cols-2 md:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-2 ${temaNoturno ? 'border-gray-700' : 'border-gray-200'}`}>
                <div className={`col-span-2 md:col-span-4 p-4 rounded-xl flex justify-between items-center ${temaNoturno ? 'bg-gray-900' : 'bg-gray-50'}`}>
                  <div>
                    <p className="text-xs uppercase font-bold text-gray-500 mb-1">Dinheiro Esperado na Gaveta</p>
                    <p className={`text-2xl font-black ${temaNoturno ? 'text-white' : 'text-gray-800'}`}>R$ {saldoGavetaEsperado.toFixed(2)}</p>
                  </div>
                </div>
                
                <div>
                  <p className="text-xs text-gray-500">Saldo Inicial</p>
                  <p className="font-bold text-gray-400">R$ {saldoInicial.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Vendas (Dinheiro)</p>
                  <p className="font-bold text-green-500">+ R$ {totalSistemaDinheiro.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Suprimentos</p>
                  <p className="font-bold text-green-500">+ R$ {totalSuprimentos.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Sangrias</p>
                  <p className="font-bold text-red-500">- R$ {totalSangrias.toFixed(2)}</p>
                </div>

                <div className="col-span-2 mt-2">
                  <p className="text-xs text-gray-500">Total Cartão (Sistema)</p>
                  <p className={`font-bold ${temaNoturno ? 'text-gray-300' : 'text-gray-700'}`}>R$ {totalSistemaCartao.toFixed(2)}</p>
                </div>
                <div className="col-span-2 mt-2">
                  <p className="text-xs text-gray-500">Total Pix (Sistema)</p>
                  <p className={`font-bold ${temaNoturno ? 'text-gray-300' : 'text-gray-700'}`}>R$ {totalSistemaPix.toFixed(2)}</p>
                </div>
              </div>
            )}
          </div>

          <button onClick={abrirConfirmacaoFechamento} className="w-full py-4 bg-purple-600 hover:bg-purple-700 text-white font-black text-lg rounded-xl transition shadow-lg">
            Finalizar Turno e Fechar Caixa
          </button>
        </>
      ) : (
        /* ABA HISTÓRICO COM EDIÇÃO DO DONO */
        <div>
              ...
              {historicoCaixas.map((caixa) => {
                ...
                return (
                  <div key={caixa.id} className={`relative p-5 rounded-2xl border ${temaNoturno ? 'bg-gray-900 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                    ...
                    {/* Botão de Editar Histórico (Dono) */}
                    ...
                    <div className="flex flex-col sm:flex-row justify-between mb-4 border-b border-gray-200/20 pb-3 gap-2 pr-16">
                      <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                      <div className={`p-3 rounded-xl border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-gray-100/50 border-gray-200'}`}>
                        <p className="text-[10px] uppercase font-bold text-gray-500">Dinheiro Apurado</p>
                        <p className={`font-black ${temaNoturno ? 'text-white' : 'text-gray-800'}`}>R$ {(caixa.relatorio_fechamento?.informadoDinheiro || 0).toFixed(2)}</p>
                        <p className="text-[10px] text-gray-500 mt-1">Sistema: R$ {(caixa.relatorio_fechamento?.esperadoDinheiro || 0).toFixed(2)}</p>
                      </div>
                      <div className={`p-3 rounded-xl border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-gray-100/50 border-gray-200'}`}>
                        <p className="text-[10px] uppercase font-bold text-gray-500">Cartão Apurado</p>
                        <p className={`font-black ${temaNoturno ? 'text-white' : 'text-gray-800'}`}>R$ {(caixa.relatorio_fechamento?.informadoCartao || 0).toFixed(2)}</p>
                        {caixa.relatorio_fechamento?.esperadoCartao !== undefined && (
                          <p className="text-[10px] text-gray-500 mt-1">Sistema: R$ {(caixa.relatorio_fechamento?.esperadoCartao || 0).toFixed(2)}</p>
                        )}
                      </div>
                      <div className={`p-3 rounded-xl border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-gray-100/50 border-gray-200'}`}>
                        <p className="text-[10px] uppercase font-bold text-gray-500">Pix Apurado</p>
                        <p className={`font-black ${temaNoturno ? 'text-white' : 'text-gray-800'}`}>R$ {(caixa.relatorio_fechamento?.informadoPix || 0).toFixed(2)}</p>
                        {caixa.relatorio_fechamento?.esperadoPix !== undefined && (
                          <p className="text-[10px] text-gray-500 mt-1">Sistema: R$ {(caixa.relatorio_fechamento?.esperadoPix || 0).toFixed(2)}</p>
                        )}
                      </div>
                      <div className={`p-3 rounded-xl border ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-gray-100/50 border-gray-200'}`}>
                        <p className="text-[10px] uppercase font-bold text-gray-500">Extra</p>
                        <p className="font-bold text-red-500 text-xs">San: R$ {(caixa.relatorio_fechamento?.sangrias || 0).toFixed(2)}</p>
                        <p className="font-bold text-green-500 text-xs mt-0.5">Sup: R$ {(caixa.relatorio_fechamento?.suprimentos || 0).toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                );
      ...
      {/* MODAIS (SANGRIA, SUPRIMENTO, SENHA, AÇÕES) */}
      {movModal.visivel && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70] animate-in fade-in">
          <div className={`rounded-3xl p-6 w-full max-w-sm shadow-2xl flex flex-col border ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
            <h2 className={`text-xl font-bold mb-4 ${temaNoturno ? 'text-white' : 'text-gray-900'}`}>
              {movModal.tipo === 'sangria' ? 'Retirada de Dinheiro' : 'Entrada de Dinheiro'}
            </h2>
      ...
      {/* MODAL DE EDIÇÃO DE FECHAMENTO (SÓ PARA O DONO) */}
      {modalEdicao.visivel && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70] animate-in fade-in">
          <div className={`rounded-3xl p-6 w-full max-w-sm shadow-2xl flex flex-col border ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
