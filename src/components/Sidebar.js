...
export default function Sidebar({
  ...
  const MenuItem = ({ id, titulo, icone, onClick, onHover, isAtivo, hoverable }) => (
    ...
    >
      ...
      {isAtivo && <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-[16px] rounded-r-full ${isDark ? 'bg-white' : 'bg-zinc-900'}`}></span>}
    </button>
  );
    ...
    <>
      ...
      `}>
         <div className="h-[68px] xl:h-[68px] px-5 flex items-center shrink-0 justify-between xl:justify-center group-hover/sidebar:xl:justify-start">
            <div className={`flex items-center shrink-0 overflow-hidden ${isDark ? 'text-white' : 'text-zinc-900'}`}>
              <span className="font-black tracking-tighter text-[24px] xl:text-[20px] leading-none whitespace-nowrap ml-3 xl:opacity-0 xl:w-0 group-hover/sidebar:xl:w-auto group-hover/sidebar:xl:opacity-100 transition-opacity duration-200 delay-75">AROX</span>
            </div>
         </div>
         ...
         <div className="mt-auto shrink-0 overflow-hidden transition-colors duration-300 pb-4 xl:pb-3">
            <div className="px-4 xl:px-3 pt-3 flex flex-col gap-1">
               <div className={`flex items-center px-3 py-2.5 rounded-[10px] xl:rounded-lg mb-2 xl:justify-center group-hover/sidebar:xl:justify-start ${isDark ? 'bg-white/[0.02] border border-white/[0.05]' : 'bg-black/[0.02] border border-black/[0.05]'}`}>
                 <div className="relative shrink-0 flex items-center justify-center w-[20px] h-[20px] xl:w-[18px] xl:h-[18px]">
                   ...
                   <span className={`absolute -bottom-0.5 -right-0.5 block h-2 w-2 rounded-full bg-emerald-500 ring-2 ${isDark ? 'ring-[#09090B]' : 'ring-[#FAFAFA]'}`}></span>
                 </div>
                 <div className="flex flex-col min-w-0 ml-3 xl:opacity-0 xl:w-0 group-hover/sidebar:xl:w-auto group-hover/sidebar:xl:opacity-100 transition-opacity duration-200 delay-75">
                    <span className={`text-[12px] font-semibold truncate leading-none tracking-tight ${isDark ? 'text-zinc-200' : 'text-zinc-800'}`}>{nomeEmpresa || 'Estabelecimento'}</span>
                    <span className={`text-[10px] flex items-center gap-1.5 font-medium truncate mt-1 tracking-wide ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>
                      {sessao?.nome_usuario || 'Usuário'}
                      <span className={`px-1 rounded-[3px] text-[8px] uppercase tracking-wider font-bold ${isDark ? 'bg-white/10 text-white' : 'bg-black/5 text-black'}`}>{(!planoEmpresa || ['free', 'grátis', 'starter'].includes(planoEmpresa.toLowerCase())) ? 'BETA' : 'PRO'}</span>
                    </span>
                 </div>
               </div>
