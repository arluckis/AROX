export default function CardComanda({ comanda, onClick, temaNoturno }) {
  ...
  const formatarAbertura = (isoDate) => {
    if (!isoDate) return { texto: '', isOutroDia: false };
    ...
    if (!isOutroDia) {
      return { texto: `Hoje às ${hora}:${min}`, isOutroDia: false };
    }
    return { texto: `${dia}/${mes} às ${hora}:${min}`, isOutroDia: true };
  };

  const abertura = formatarAbertura(comanda.hora_abertura);
  const isDelivery = comanda.tipo === 'Delivery';

  // Cores de fundo específicas por tipo de comanda
  const bgClass = temaNoturno 
  ...
  return (
    <button 
      onClick={onClick} 
      className={`w-44 h-48 rounded-3xl p-4 flex flex-col hover:shadow-xl transition-all duration-300 cursor-pointer text-left relative active:scale-95 border shadow-sm ${bgClass}`}
    >
      <div className="flex flex-col mb-2 w-full gap-1">
        <div className="flex justify-between items-start w-full gap-2">
          {/* text-sm usado aqui para diminuir um pouco o título */}
