...
export default function TabFidelidade({ temaNoturno, sessao, mostrarAlerta, mostrarConfirmacao, dadosTags, clientesFidelidade, setClientesFidelidade, comandas }) {
  ...
  // ADICIONADO O valor_minimo AQUI NO ESTADO INICIAL
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      ...
      // ADICIONADO O valor_minimo AQUI PARA PUXAR DO BANCO
  ...
  const atualizarMeta = async () => {
    // ADICIONADO O valor_minimo AQUI PARA SALVAR NO BANCO
    ...
    try {
      ...
      if (novos.length > 0) {
        ...
        if (data && !error) {
          ...
          setTextoImportacao('');
          mostrarAlerta("Sucesso", `${data.length} clientes importados com sucesso para o Banco!`);
        } else {
          mostrarAlerta("Erro", "Erro ao salvar no banco: " + error.message);
        }
      } else { mostrarAlerta("Aviso", "Nenhum cliente válido encontrado. Verifique a formatação."); }
    } catch(e) { mostrarAlerta("Erro", "Ocorreu um erro ao processar sua tabela. Verifique se os dados estão corretos."); }
  };
    ...
    <div className="flex-1 animate-in slide-in-from-bottom-4 duration-500 w-full max-w-7xl mx-auto px-2 lg:px-0 pb-10">
      ...
      <div className={`p-6 rounded-3xl shadow-sm border mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <button onClick={() => { setClienteEditando(null); setNovoCliente({nome:'', telefone:'', aniversario:'', pontos:0}); setMostrarModalNovo(true); }} className="flex-1 md:flex-none px-4 py-2.5 rounded-xl font-bold text-xs uppercase bg-purple-600 hover:bg-purple-700 text-white shadow-md transition">
            + Cadastrar Cliente
          </button>
        </div>
      </div>
                  ...
                  {clientesFiltrados.map(c => {
                    ...
                    return (
                      <tr key={c.id} className={`border-b last:border-0 transition-colors ${temaNoturno ? 'border-gray-800 hover:bg-gray-800/50 text-gray-300' : 'border-gray-50 hover:bg-purple-50/50 text-gray-700'}`}>
                        <td className="p-4">
                          <p onClick={() => setClientePerfil(c)} title="Ver Histórico do Cliente" className={`font-black text-sm uppercase cursor-pointer hover:underline ${temaNoturno ? 'text-gray-200 hover:text-white' : 'text-gray-900 hover:text-purple-700'}`}>{c.nome}</p>
                          {(atingiuMeta || acimaDeDez) && <p className={`text-[10px] font-bold mt-0.5 uppercase tracking-widest ${textClass}`}>Apto a Prêmio</p>}
                        </td>
                        ...
                        <td className="p-4 text-right">
                          <div className="flex justify-end gap-2">
                             <button onClick={() => resgatarPremio(c)} title="Descontar pontos da carteira do cliente e entregar prêmio" className={`px-3 py-1.5 rounded-lg text-xs font-black transition uppercase tracking-wider ${c.pontos >= meta.pontos_necessarios ? 'bg-green-500 text-white hover:bg-green-600 shadow-sm' : (temaNoturno ? 'bg-gray-800 text-gray-600 cursor-not-allowed' : 'bg-gray-100 text-gray-400 cursor-not-allowed')}`} disabled={c.pontos < meta.pontos_necessarios}>Resgatar</button>
                             <button onClick={() => abrirEdicao(c)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition border ${temaNoturno ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}>Editar</button>
                          </div>
                        </td>
                      </tr>
                    )
      ...
      {abaInterna === 'ranking' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-6 rounded-3xl border shadow-sm ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
            <h3 className={`font-black text-sm mb-2 uppercase tracking-widest flex items-center gap-2 ${temaNoturno ? 'text-white' : 'text-gray-800'}`}>
      ...
      {abaInterna === 'config' && (
        <div className={`p-6 rounded-3xl border shadow-sm max-w-lg ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="text-xs font-black uppercase tracking-widest text-gray-500 mb-2 block">Valor Mínimo p/ Pontuar (R$)</label>
              <input type="number" value={meta.valor_minimo} onChange={e => setMeta({...meta, valor_minimo: e.target.value})} className={`w-full p-3 rounded-xl border outline-none font-bold text-sm transition focus:border-purple-500 ${temaNoturno ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'}`} />
            </div>
            <div>
            ...
            <button onClick={atualizarMeta} className="w-full py-4 mt-4 rounded-xl font-black uppercase text-xs tracking-widest bg-purple-600 hover:bg-purple-700 text-white shadow-md active:scale-95 transition-all">
              Salvar Diretrizes
            </button>
          </div>
        </div>
      ...
      {abaInterna === 'publico' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-6 rounded-3xl shadow-sm border flex flex-col h-[400px] ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
            <h3 className={`text-sm font-black uppercase tracking-widest mb-2 text-center ${temaNoturno ? 'text-white' : 'text-gray-900'}`}>Receita: Fiéis x Avulsos</h3>
            ...
            ) : <div className={`flex-1 flex items-center justify-center text-sm font-bold ${temaNoturno ? 'text-gray-600' : 'text-gray-400'}`}>Nenhuma comanda processada.</div>}
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className={`w-full max-w-md p-8 rounded-3xl border shadow-2xl ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
            ...
            <div className="flex gap-3 mt-8">
              <button onClick={() => { setMostrarModalNovo(false); setClienteEditando(null); }} className={`flex-1 py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest transition ${temaNoturno ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Cancelar</button>
              <button onClick={salvarNovoCliente} className="flex-1 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest bg-purple-600 text-white shadow-md hover:bg-purple-700 active:scale-95 transition">{clienteEditando ? 'Atualizar' : 'Salvar'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className={`w-full max-w-md p-8 rounded-3xl border shadow-2xl ${temaNoturno ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
            <div className="flex justify-between items-start mb-6">
               <div>
                 ...
                 <p className={`text-xs font-bold uppercase tracking-widest mt-1 ${temaNoturno ? 'text-purple-400' : 'text-purple-600'}`}>{clientePerfil.nome}</p>
               </div>
               <button onClick={() => setClientePerfil(null)} className={`p-2 rounded-full transition ${temaNoturno ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>✕</button>
            </div>
            ...
            <h3 className={`text-sm font-black uppercase tracking-widest mb-4 flex items-center gap-2 ${temaNoturno ? 'text-gray-300' : 'text-gray-800'}`}>
            ...
            {produtosDoCliente.length === 0 ? (
               <p className={`text-xs font-bold text-center p-6 rounded-xl border border-dashed ${temaNoturno ? 'text-gray-500 border-gray-700' : 'text-gray-400 border-gray-300'}`}>Nenhuma comanda vinculada a este nome encontrada no histórico recente.</p>
            ) : (
               <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2">
                  {produtosDoCliente.map((p, idx) => (
                     <div key={idx} className={`p-3 rounded-xl border flex justify-between items-center ${temaNoturno ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200 shadow-sm'}`}>
                        <div className="flex items-center gap-3">
                           ...
                           <span className={`text-xs font-black uppercase ${temaNoturno ? 'text-gray-200' : 'text-gray-800'}`}>{p.nome}</span>
                        </div>
