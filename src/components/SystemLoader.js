...
export default function SystemLoader({ variant = 'section', text = '', phase = 'sync', exitStage = 'none' }) {
  ...
  if (variant === 'full') {
    ...
    return (
      <div className={`absolute inset-0 z-50 w-full h-full flex flex-col items-center justify-center perspective-[1200px] transition-all duration-[1200ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${
        ...
        <div className={`flex flex-col items-center justify-center p-12 transition-all duration-[800ms] ease-out min-w-[320px] min-h-[320px] relative overflow-hidden ${
          ...
          <div className={`absolute top-8 flex flex-col items-center justify-center w-full transition-all duration-[800ms] ease-out ${isExitingArox ? 'opacity-0 scale-90 blur-sm' : 'opacity-100 scale-100 blur-0'}`}>
             <span className="text-[14px] font-bold tracking-[0.5em] text-white uppercase mb-1 drop-shadow-[0_0_12px_rgba(255,255,255,0.4)]">
               AROX
             </span>
             <span className={`text-[9px] font-medium tracking-[0.4em] text-zinc-500 uppercase transition-opacity duration-500 ${isExitingContent ? 'opacity-0' : 'opacity-100'}`}>
               Ignition Sequence
             </span>
          </div>

          <div className={`flex flex-col items-center justify-center mt-6 transition-all duration-500 ${isExitingContent ? 'opacity-0 scale-90 blur-sm' : 'opacity-100 scale-100 blur-0'}`}>
            <AnimatedBars isPremium={true} />
          </div>

          <div className={`absolute bottom-10 flex justify-center w-full transition-all duration-500 ${isExitingContent ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
